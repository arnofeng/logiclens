...
export async function callEdgesAround(db: GraphDB, codeIds: string[]): Promise<EdgeRow[]> {
  ...
  return db.query<EdgeRow>(
    `MATCH (fromFile:File)-[:CONTAINS]->(a:Code)-[r:CALLS]->(b:Code)<-[:CONTAINS]-(toFile:File)
     ...
     RETURN a.qualifiedName AS fromName, b.qualifiedName AS toName, fromFile.path AS fromFile, toFile.path AS toFile, r.confidence AS confidence,
