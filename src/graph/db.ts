...
import { schemaStatements, systemId } from "./schema.js";
...
export class KuzuGraphDB implements GraphDB {
  ...
  async cleanupGraphWriteBatch(batchId: string): Promise<void> {
    ...
    // Recovery is intentionally a visibility cleanup instead of a destructive
    // rollback: writer modes use upserts/COPY across multiple statements, so
    // marking batch-owned artifacts inactive is the idempotent cross-writer
    // boundary that keeps default queries from seeing a partial write.
  ...
  async close(): Promise<void> {
    ...
    if (shouldUseManagedKuzuClose()) {
      // On Windows, explicit close or early native-object finalization can crash
      // the current Kuzu Node binding. Keep references alive and let process
      // teardown release them, while still preventing further use.
