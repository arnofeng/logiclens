...
export const tsSchemaExtractor: ContractExtractor = {
  ...
  /**
   * Post-extract: resolve pending USES_SCHEMA edges to actual spec IDs.
   * The per-file extract() phase creates stub edges; this hook looks up
   * the actual ContractSpec IDs from the merged fact set.
   */
  postExtract(context) {
    const result = createCrossRepoExtraction();

    // Build a map from schema name → specId for resolution
    ...
    for (const spec of context.mergedFacts.contractSpecs) {
      ...
      const parsed = safeJsonParse(spec.specJson) as SchemaSpec | null;
      if (parsed?.name) {
        schemaNameToSpecId.set(parsed.name.toLowerCase(), spec.id);
      }
    }

    // Also map contract key → spec ID (canonicalContractKey lowercases)
    ...
    // Resolve pending USES_SCHEMA edges
    for (const rel of context.mergedFacts.semanticRelations) {
      ...
      // Find the actual fromSpecId by matching the contract
    ...
    // Also establish REQUEST_SCHEMA / RESPONSE_SCHEMA relations from
    // Spring MVC endpoints that reference DTO/schema types in @RequestBody
    // or return types. These are detected from the HttpEndpointSpec's
    // requestBodyType / responseBodyType fields.
    for (const spec of context.mergedFacts.contractSpecs) {
      if (spec.specKind !== "http-endpoint") continue;
      const parsed = safeJsonParse(spec.specJson) as { requestBodyType?: string; responseBodyType?: string } | null;
      if (!parsed) continue;

      if (parsed.requestBodyType) {
        const schemaSpecId = schemaNameToSpecId.get(parsed.requestBodyType.toLowerCase());
        if (schemaSpecId) {
          result.semanticRelations.push({
            fromSpecId: spec.id,
            toSpecId: schemaSpecId,
            kind: "REQUEST_SCHEMA",
            evidenceId: spec.evidenceId,
            reason: `@RequestBody type ${parsed.requestBodyType}`,
            confidence: confidenceFor("heuristic-request-body-type")
          });
        }
      }

      if (parsed.responseBodyType) {
        const schemaSpecId = schemaNameToSpecId.get(parsed.responseBodyType.toLowerCase());
        if (schemaSpecId) {
          result.semanticRelations.push({
            fromSpecId: spec.id,
            toSpecId: schemaSpecId,
            ...
            evidenceId: spec.evidenceId,
            reason: `Response type ${parsed.responseBodyType}`,
            confidence: confidenceFor("heuristic-request-body-type")
          });
        }
      }
