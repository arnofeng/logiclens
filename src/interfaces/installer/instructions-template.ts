...
- **MCP tools** (when available): \`${brandedMcpToolName("get_stats")}\` reports node and relation counts. \`${brandedMcpToolName("list_dependencies")}\` and \`${brandedMcpToolName("list_contracts")}\` summarize relations and endpoints. \`${brandedMcpToolName("trace")}\` traces multi-hop semantic contract connections (schemas, consumers, events). \`${brandedMcpToolName("impact_analysis")}\` evaluates the downstream blast radius. Use the host's native file and code search for free-text implementation discovery. If tools are deferred, load them by name.
- **Shell** (always works): \`${BRAND.cliName} impact <symbolOrEntity>\` and \`${BRAND.cliName} trace "<contract>"\` (e.g. \`${BRAND.cliName} trace "http POST /orders"\`, \`${BRAND.cliName} trace "event OrderCreated"\`) print the same output. Use the host's native file search for free-text source exploration.
