...
export async function runIndexStateCommitPhase(input: {
  ...
  const { db, repo, batchId, indexedAt, filesScanned, filesChanged, filesStale, status, summaryFailures, graphWriteAtomicity, graphWriteStatus, error } = input;
  await runIndexPhase({ phase: "index-state-commit", repoName: repo.name, repoId: repo.id, batchId }, async () => {
    ...
    const stateError = status === "failed"
      ? errorMessage(error)
      : combineIndexWarnings(formatSummaryWarning(summaryFailures));
