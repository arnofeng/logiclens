...
describe("Neo4j schema quality conformance", () => {
  it.skipIf(!integration.enabled)(integration.testName, async () => {
    ...
    const workspaceId = `workspace:schema-quality:${suffix}`;
    ...
    let failure: unknown;
    try {
      await db.initSchema(`schema-quality-${suffix}`);
      await runSchemaQualityConformance(db, workspaceId);
    } catch (error) {
      failure = error;
    }
    ...
    try {
      await runNeo4jCleanupSteps([
        { name: "delete schema quality facts", run: () => db.query(
