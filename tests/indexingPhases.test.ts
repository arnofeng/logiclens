...
describe("indexing phases", () => {
  it("returns a phase result with duration for successful work", async () => {
    const result = await runIndexPhase({ phase: "scan", repoName: "service-a" }, () => ({ files: 3 }));
  ...
  it("wraps failures in IndexPhaseError with phase context", async () => {
    await expect(runIndexPhase(
      { phase: "parse", repoName: "service-a", filePath: "src/OrderService.ts" },
